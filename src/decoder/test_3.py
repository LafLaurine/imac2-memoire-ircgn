...
from tensorflow.keras.layers import Dense, Flatten, Reshape, Input, Conv2D, Conv2DTranspose, MaxPooling2D
...
decoder.summary()
decoder.fit(x=A_train, y=A_train, epochs=2,
                    validation_data=[A_test[0], A_test[0]])
...
if os.path.isfile('autoencoder_B.json') and os.path.isfile('autoencoder_B_weights.hdf5'):
    ...
    autoencoder_B = Model(inp,reconstruction)autoencoder_B.compile(optimizer='adamax', loss='mse')print(autoencoder_B.summary())###################  DATA AUTOENCODERA_train, A_test = train_test_split(A, test_size=0.1, random_state=42)DATA_SHAPE = (A.shape[1:] + (1,))
print(DATA_SHAPE)
encoder_A, decoder_A = build_autoencoder(DATA_SHAPE, CODE_SIZE)inp = Input((DATA_SHAPE))
code = encoder_A(inp)
reconstruction = decoder_A(code)if os.path.isfile('autoencoder_A.json') and os.path.isfile('autoencoder_A_weights.hdf5'):
    # load json and create model
    ...
    autoencoder_A = Model(inp,reconstruction)autoencoder_A.compile(optimizer='adamax', loss='mse')print(autoencoder_A.summary())def visualize_B(img,encoder,decoder):
    """Draws original, encoded and decoded images"""
    # img[None] will have shape of (1, 512, 512, 3) which is the same as the model input
    code = encoder_B.predict(img[None])[0]
    reco = decoder_B.predict(code[None])[0]    plt.subplot(1,3,1)
    plt.title("Original")
    show_image(img)    plt.subplot(1,3,2)
    plt.title("Code")
    plt.imshow(code.reshape([code.shape[-1]//2,-1]))    plt.subplot(1,3,3)
    plt.title("Reconstructed")
    show_image(reco)
    plt.show()def visualize_A(img,encoder,decoder):
    """Draws original, encoded and decoded images"""
    # img[None] will have shape of (1, 512, 512, 3) which is the same as the model input
    code = encoder_A.predict(img[None])[0]
    reco = decoder_A.predict(code[None])[0]    plt.subplot(1,3,1)
    plt.title("Original")
    show_image(img)    plt.subplot(1,3,2)
    plt.title("Code")
    plt.imshow(code.reshape([code.shape[-1]//2,-1]))    plt.subplot(1,3,3)
    plt.title("Reconstructed")
    show_image(reco)
    plt.show()   def visualize_A_with_B(img,encoder,decoder):
    """Draws original, encoded and decoded images"""
    # img[None] will have shape of (1, 512, 512, 3) which is the same as the model input
    code = encoder_A.predict(img[None])[0]
    reco = decoder_B.predict(code[None])[0]    plt.subplot(1,3,1)
    ...
    plt.title("Reconstructed")
    show_image(reco)
    plt.show()     def apply_gaussian_noise(X, sigma=0.1):
    noise = np.random.normal(loc=0.0, scale=sigma, size=X.shape)
    return X + noisefor i in range(3):
    print("Epoch %i/25, Generating corrupted samples..."%(i+1))
    autoencoder_B.fit(x=X_train, y=X_train, epochs=2,
                    validation_data=[X_test, X_test])for i in range(3):
    img = X_test[i]
    visualize_B(img,encoder_B,decoder_B)##################################################""for i in range(200):
    print("Epoch %i/200, Generating corrupted samples..."%(i+1))    autoencoder_A.fit(x=A_train, y=A_train, epochs=2,
                    validation_data=[A_test, A_test])for i in range(3):
    img = A_test[i]
    visualize_A(img,encoder_A,decoder_A)for i in range(3):
    img = A_test[i]
    visualize_A_with_B(img,encoder_A,decoder_B)def save_model():  def save(model, model_name):
      ...
      model.save_weights(options['file_weight'])  save(autoencoder_A, "autoencoder_A")
  save(autoencoder_B, "autoencoder_B")save_model()'''