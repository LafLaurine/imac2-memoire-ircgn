...
with open('csv/'+base+'.csv', 'w') as outfile:  
    ...
    csvwriter.writerow(['File_name','Lips_distance', 'Theta','Phi','Psi','Expression1','Expression2','Expression3','Expression4','Expression5','Expression6','Expression7','Center','Bounding_box'])
    for i in range(len(col_lips)):
        '''nb = col_angles[i]
        nb[0] = round(nb[0],precision)
        nb[1] = round(nb[1],precision)
        nb[2] = round(nb[2],precision)
        input_angles = [nb[0],nb[1],nb[2]]

        nbExpr = col_expression[i]
        nbExpr[0] = round(nbExpr[0],precision)
        nbExpr[1] = round(nbExpr[1],precision)
        nbExpr[2] = round(nbExpr[2],precision)
        nbExpr[3] = round(nbExpr[3],precision)
        nbExpr[4] = round(nbExpr[4],precision)
        nbExpr[5] = round(nbExpr[5],precision)
        nbExpr[6] = round(nbExpr[6],precision)
        ...
        csvwriter.writerow([onlyfiles[i],col_lips[i],col_theta[i],col_lips[i],col_psi[i],col_expression1[i],col_expression2[i],col_expression3[i],col_expression4[i],col_expression5[i],col_expression6[i],col_expression7[i],input_center,col_bounding[i]])
